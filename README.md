...
| 通常表示 | 日付表示 |
| --- | --- |
| ![通常表示](docs/screenshots/normal.svg) | ![日付表示](docs/screenshots/date-display.svg) |

| 時計のみ表示 | 色設定 |
| --- | --- |
| ![時計のみ表示](docs/screenshots/clock-only.svg) | ![色設定](docs/screenshots/settings.svg) |

## ダウンロード

GitHub Releasesから `SevenSegmentClock-v1.2.1.zip` をダウンロードし、ZIPを展開して `SevenSegmentClock.exe` を実行してください。
...
```text
release\SevenSegmentClock-v1.2.1.zip
```
...
曜日色をONにすると、年月日＋曜日表示のときに土曜日は青、日曜日は赤で表示します。
...
`tools\create_icon.py` は画像を描き直さず、PNGをICOコンテナへ格納するためのスクリプトです。
